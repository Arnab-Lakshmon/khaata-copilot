...
async function generatedReasoning(transaction: Pending, candidate: FuzzyDecision) {
  ...
  const facts = { payer: transaction.payerName || transaction.transaction.rawReference, invoiceParty: candidate.invoice.party_name, invoiceNumber: candidate.invoice.invoice_number, transactionAmount: transaction.transaction.amount, invoiceRemainingBalance: Number(candidate.invoice.remainingBalance), nameSimilarity: candidate.nameSimilarity, amountSimilarity: candidate.amountSimilarity, dateDifferenceDays: candidate.dateDifferenceDays, possiblePartial: candidate.possiblePartial, possibleSplit: candidate.possibleSplit, confidence: candidate.confidence };
